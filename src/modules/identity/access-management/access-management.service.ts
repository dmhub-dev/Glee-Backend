...
export class AccessManagementService {
  ...
  async inviteUser(dto: InviteUserDto, actor: any, requestMeta: Record<string, string | undefined>) {
    ...
    const pendingInvite = await this.prisma.userInvitation.findFirst({
      where: { email: dto.email, status: 'PENDING', expiresAt: { gt: new Date() } },
      select: { id: true },
    });
    if (pendingInvite) throw new HttpException('A pending invitation already exists for this email', HttpStatus.BAD_REQUEST);
  ...
  async listInvitations() {
    ...
    return { success: true, message: 'Invitations retrieved successfully', data: invitations.map(i => this.withInvitationLink(i)) };
  }
  ...
  async resendInvitation(id: string, actor: any) {
    ...
    if (invitation.status !== 'PENDING') throw new HttpException('Only pending invitations can be resent', HttpStatus.BAD_REQUEST);
    ...
    return { success: true, message: 'Invitation resent successfully', data: this.withInvitationLink(updated) };
  }

  async revokeInvitation(id: string, actor: any) {
    const invitation = await this.prisma.userInvitation.findUnique({ where: { id } });
    if (!invitation) throw new HttpException('Invitation not found', HttpStatus.NOT_FOUND);
    if (invitation.status !== 'PENDING') throw new HttpException('Only pending invitations can be revoked', HttpStatus.BAD_REQUEST);
    ...
    return { success: true, message: 'Invitation revoked successfully', data: updated };
  }
  ...
  async acceptInvitation(token: string, dto: AcceptInvitationDto) {
    ...
    if (invitation.expiresAt < new Date()) {
      await this.prisma.userInvitation.update({ where: { id: invitation.id }, data: { status: 'EXPIRED' } });
    ...
    return { success: true, message: 'Invitation accepted successfully', data: result };
  }
  ...
  async listUsers(query: ListUsersQueryDto) {
    ...
    return { success: true, message: 'Users retrieved successfully', data: { items, total, page, limit } };
  }
  ...
  async updateUser(id: string, dto: UpdateUserDto, actor: any) {
    ...
    await this.log(actor, 'users.update', 'User', id, { beforeRole: current.role?.name, afterRole: dto.role });
  ...
  async deleteUser(id: string, actor: any) {
    ...
    await this.prisma.user.update({
      where: { id },
      data: { isDeleted: true, deletedAt: new Date(), isActive: AccountStatus.INACTIVE },
    });
  ...
  async updateRolePermissions(roleName: UserRole, dto: UpdateRolePermissionsDto, actor: any) {
    ...
    if (missing.length > 0) {
      throw new HttpException(`Unknown permissions: ${missing.join(', ')}`, HttpStatus.BAD_REQUEST);
    }
    ...
    await this.log(actor, 'roles.permissions_update', 'Role', role.id, { role: roleName, permissions: dto.permissions });
  ...
  async listAuditLogs(query: ListAuditLogsQueryDto) {
    ...
    return { success: true, message: 'Audit logs retrieved successfully', data: { items, total, page, limit } };
  }
  ...
  private invitationLink(token: string) {
    const baseUrl = this.config.get<string>('ADMIN_APP_URL') ?? this.config.get<string>('APP_URL') ?? 'http://localhost:8003';
