...
export class EventSharedService {
  ...
  async getEventTicketsData(_filter: any, _pagination?: IPaginationOptions, _options?: IFetchEventTicketOptions) {
    return [];
  }
  ...
  async helperGetEventParticipants(_filter: { eventId: string; userId: string }, _me: any) {
    return { data: [], count: 0 };
  }

  async helperEventFindById(id: string) {
    return this.prisma.event.findFirst({ where: { id, isDeleted: false } });
  }
